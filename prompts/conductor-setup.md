---
description: Set up Conductor environment for spec-driven development
argument-hint:
---
...
2.  **Resume Based on State:**
    ...
    - If `STEP` is "3.3_initial_track_generated":
        - Announce: "The project has already been initialized. You can create a new track with `/prompts:conductor-new-track` or start implementing existing tracks with `/prompts:conductor-implement`."
...
1.  **Initiate Dialogue:** Announce that the initial scaffolding is complete and you now need the user's input to select the project's code style guides.
2.  **Select Code Style Guides:**
    -   Based on the Tech Stack defined in the previous step, recommend the most appropriate style guide(s) and explain why.
    ...
    -   If the user chooses to edit (Option B):
        -   Present a list of common style guides (general, javascript, typescript, python, go, html-css).
        -   Ask the user which guide(s) they would like to include.
    -   **Action:** Create the `conductor/code_styleguides/` directory and write the selected style guide files.
...
1.  **Copy Initial Workflow:**
    -   Create `conductor/workflow.md` with a default TDD-based workflow.
...
3.  **Generate Track Artifacts:**
    ...
    c. **Create Track Artifacts:**
        i. **Generate and Store Track ID:** Create a unique Track ID from the track description using format `shortname_YYYYMMDD` and store it.
        ii. **Create Single Directory:** Using the stored Track ID, create a single new directory: `conductor/tracks/<track_id>/`.
        iii. **Create `metadata.json`:** In the new directory, create a `metadata.json` file with the correct structure and content.
    ...
    e. **Announce Progress:** Announce that the track has been created.
...
3.  **Next Steps:** Inform the user that they can now begin work by running `/prompts:conductor-implement`.
