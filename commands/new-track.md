---
description: Plans a new track, generates track-specific spec documents and updates the tracks file
---
...
2. **Generate Track ID:** Create a unique Track ID (e.g., `shortname_YYYYMMDD`). **SECURITY:** The `shortname` MUST consist only of lowercase alphanumeric characters, hyphens, and underscores (`[a-z0-9_-]`), with a maximum of 40 characters. Strip or replace any other characters (spaces → `_`, remove special chars). Never use user-provided text verbatim as a path component without this sanitization.
