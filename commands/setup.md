...
0. **Check for Incompatible Conductor Structure:** Before checking the state file, verify that there is no pre-existing conductor layout that uses a different index format. If `conductor/index.md` exists but `conductor/tracks.md` does NOT exist, halt and inform the user: "A conductor directory with a different structure (`conductor/index.md`) was detected. This may be from a different conductor variant. Proceeding would create a parallel `conductor/tracks.md` and cause conflicts. Please ensure you are using the correct conductor implementation for this project, or manually migrate `conductor/index.md` to `conductor/tracks.md` before running setup."
   ...
   - **If Brownfield:**
       ...
       - **Begin Brownfield Project Initialization Protocol:**
           ...
           - **2.1 File Size and Relevance Triage:**
               ...
               3. **Fallback to Manual Ignores:** ONLY if neither `.claudeignore` nor `.gitignore` exist, you should fall back to manually ignoring common directories. Use a cross-platform `find` command (avoid `ls -I`, which is GNU-only and fails on macOS). Example: `find . -type f -not -path '*/node_modules/*' -not -path '*/.m2/*' -not -path '*/build/*' -not -path '*/dist/*' -not -path '*/bin/*' -not -path '*/target/*' -not -path '*/.git/*' -not -path '*/.idea/*' -not -path '*/.vscode/*'`.
...
3. **Generate Track Artifacts:**
   ...
   c. **Create Track Artifacts:**
       ...
       iii. **Create `metadata.json`:** In the new directory, create a `metadata.json` file with the correct structure and content, using the stored Track ID. An example is:
           ...
           ```
           Valid values for `type`: `"feature"`, `"bug"`, `"chore"`, `"refactor"`.
           Valid values for `status`: `"new"`, `"in_progress"`, `"completed"`, `"cancelled"`.
